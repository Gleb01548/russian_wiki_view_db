...
dag = DAG(
    dag_id="test_drop_and_create_tables",
    tags=[
        "test",
        "wikipedia_views",
        "drop_table",
...
# create_table_group = [i for i in create_table_group]

# new_str = ""

# len_create_table_group = len(create_table_group) - 1

# for index, _ in enumerate(create_table_group):
#     if index < len_create_table_group:
#         new_str += f"create_table_group[{index}] >> "
#     else:
#         new_str += f"create_table_group[{index}]"


# eval(f"{new_str}") >> end_week
