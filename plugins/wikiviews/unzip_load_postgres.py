...
class UnzipLoadPostgres(BaseOperator):
    ...
    def execute(self, context: Context) -> None:
        ...
        script_list = [
            f"insert into resource.{self.domain_code} (page_name, page_view_count, datetime, domain_code) values\n",
        ]
        ...
        for index, (pagename, page_view_count) in enumerate(result.items()):
            ...
            script_list.append(
                ...
                f" '{actual_time}', '{self.domain_code}'){symbol}"
            )
        script = "\n".join(script_list)
        logging.info("Запись данных в БД")
